...
import type { CommentaryTone } from './commentaryState.ts';
...
export function Money({
  ...
  readonly value: Cents | null | undefined;
  readonly signed?: boolean;
}): ReactNode {
  return (
    <span className="money">
      {value === null || value === undefined ? '—' : formatMoney(value, { signed })}
    </span>
  );
}
...
export function LatencyBadge({
  ...
  const ok = ms <= budget;
  return (
    <span className={`pill ${ok ? 'status-good' : 'status-moderate'}`}>
      {ms}ms / {budget}ms
    </span>
  );
}
...
export function ProvenanceBadge({
  ...
  return (
    <span className={`pill provenance-badge provenance-${tone}`}>
...
export function MathTrace({
  steps,
}: { readonly steps: readonly Step[] } | { readonly steps: Traced<unknown>['steps'] }): ReactNode {
  return (
    <details className="math-trace">
      <summary>Show the math</summary>
      {steps.length === 0 ? (
        <p className="muted">This value was entered directly.</p>
      ) : (
        <ol>
          {steps.map((step, index) => (
            <li key={`${step.label}-${index}`}>
              ...
              {step.note === undefined ? null : <p>{step.note}</p>}
            </li>
          ))}
        </ol>
      )}
