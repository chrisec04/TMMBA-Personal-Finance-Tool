  ...
  if (snapshot === null || report === null)
    return (
      <EmptyState title="No snapshot yet">
        Load demo data or connect storage to see a financial snapshot.
      </EmptyState>
    );
  ...
  return (
    <>
      <ScreenTitle
        ...
        description={`Balances dated ${snapshot.date}. ${daysBetween(snapshot.date, asOf)} day(s) since last update.`}
      ...
      <div className="hero-strip">
        <div>
          ...
          <strong>
            <Money value={report.totalCash.value} />
          </strong>
          <MathTrace steps={report.totalCash.steps} />
        </div>
        <div>
          <span>Total debt</span>
          <strong>
            <Money value={totalDebt(state.accounts, snapshot)} />
          </strong>
        </div>
        <div>
          <span>Net worth</span>
          <strong>
            <Money value={netWorth(state.accounts, snapshot)} />
          </strong>
        </div>
        <div>
          ...
          <tbody>
            {report.accounts.map((entry) => (
              <tr key={entry.account.id}>
                ...
                <td className="num">
                  <Money value={entry.cushion} />
                </td>
                <td className="num">
                  <Money value={entry.headroom} />
                </td>
                <td>
                  <StatusPill status={entry.status} />
                </td>
                <td>
                  <MathTrace steps={entry.explanation.steps} />
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <tbody>
            {state.accounts.map((account) => (
              <tr key={account.id}>
                ...
                <td>{account.kind}</td>
                <td className="num">
                  <Money value={snapshot.balances[account.id]} />
                </td>
              </tr>
            ))}
          </tbody>
